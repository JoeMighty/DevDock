         ...
         {categories.map(cat => (
             <button
               ...
               className={cn(
                  ...
                  activeCategory === cat ? "text-white" : "text-muted-foreground hover:text-white hover:bg-muted/50"
               )}
             >
                 {activeCategory === cat && (
                     <motion.div layoutId="home-tab" className="absolute inset-0 bg-primary/20 border border-primary/30 rounded-full -z-10" />
                 )}
                 {cat}
             </button>
         ))}
            ...
            {filteredTools.map((tool) => {
              ...
              return (
                <motion.div key={tool.path} layout variants={itemVariants} className="h-full block">
                  <Link 
                    to={tool.path}
                    className="group relative flex flex-col h-full overflow-hidden p-5 bg-card/40 backdrop-blur-sm border border-border/50 rounded-xl hover:border-primary/50 transition-all duration-500 hover:shadow-[0_0_30px_rgba(139,92,246,0.15)] hover:-translate-y-1"
                  >
                    <div className="absolute inset-0 bg-gradient-to-br from-primary/5 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                    
                    <div className="flex justify-between items-start mb-3">
                        <div className="w-10 h-10 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center group-hover:scale-110 group-hover:bg-primary/20 group-hover:border-primary/30 transition-all duration-300">
                          <Icon className="w-5 h-5 text-white group-hover:text-primary drop-shadow-md" />
                        </div>
                        <span className="text-[10px] font-bold tracking-widest uppercase px-2 py-0.5 rounded-full bg-muted/50 text-muted-foreground/70">{tool.category}</span>
                    </div>
